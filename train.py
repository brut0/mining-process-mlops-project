...
AGG_FEATURES = [ # Do not use '% Iron Concentrate' because of highly correlated (0.8) with target and do not use datetime
...
models = {
    ...
    'Ridge Regression': {
        ...
        'params': {
            ...
            'tol': [0.0000001] #(0.0000001, 0.000001, 0.00001),
        }
    },
    ...
    #     'model': ElasticNet(),
    #     'params': {
    #         'alpha': (0.01, 0.05, 0.1, 1.0, 10, 100),
    #         'l1_ratio': (0.1, 0.5, 0.75, 1.0),
...
def init_mlflow():
    mlflow.set_tracking_uri(f"http://{os.environ['MLFLOW_SERVER_HOST']}:{os.environ['MLFLOW_SERVER_PORT']}")
...
def get_list_data(source='local'):
    if source == 'local':
        ...
        s3 = init_s3_client()
        files = [key['Key'] for key in s3.list_objects(Bucket='kaggle-mining-process')['Contents']]
    return files
...
def get_train_data(month, year=str(YEAR)) -> pd.DataFrame:
    s3_client = init_s3_client() # TODO
    all_data = get_list_data(source='s3')
    filenames = [file for file in all_data if file <= f"mining_data_{year}-{month}.parquet"]
    df = pd.concat(
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    X = df.groupby(pd.Grouper(key='datetime', axis=0, freq='H')).agg(create_feature_dict).reset_index().dropna()
    X.columns = ['_'.join(col) for col in X.columns]
    y = df[['datetime', TARGET]].groupby(pd.Grouper(key='datetime', axis=0, freq='H')).agg('last').reset_index().dropna()
...
if __name__ == '__main__':
    ...
    for m in models:
        with mlflow.start_run(run_name=f"GridSearchCV_{m}"):
            model = models[m]['model']
            params = models[m]['params']

            clf = GridSearchCV(model, params, cv=N_FOLDS, scoring=SCORING, return_train_score=False, n_jobs=N_JOBS)
            ...
            for metric in test_metrics:
                logger.info(f"{metric}: {test_metrics[metric]}")
