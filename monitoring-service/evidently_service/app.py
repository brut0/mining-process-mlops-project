...
Metrics calculation results are available with `GET /metrics`
 HTTP method in Prometheus compatible format.
"""
import dataclasses
import datetime
import logging
...
import yaml
from evidently.model_monitoring import ModelMonitoring, RegressionPerformanceMonitor
    ...
    def iterate(self, dataset_name: str, new_rows: pd.DataFrame):
        ...
        for metric, value, labels in self.monitoring[dataset_name].metrics():
            ...
            try:
                ...
                logging.error("Value error for metric %s, error: %i", metric_key, error)
